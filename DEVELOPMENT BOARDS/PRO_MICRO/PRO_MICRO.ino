/*
  ...
  on an **Arduino Pro Micro** to verify the functionality of key components, including:
  ...
  Usage:
  - **Before Uploading:** Select the correct **port** and set the board type to **Arduino Pro Micro**.
...
#define LED_PIN 17  // Built-in LED on Pro Micro
...
void setup() {
    ...
    if (ENABLE_DIGITAL_IO) {
        ...
        for (int pin = 2; pin <= 21; pin++) {
            if (pin == LED_PIN) continue; // Skip built-in LED pin
            pinMode(pin, OUTPUT);
    ...
    Serial.println("\n✅ Arduino Pro Micro Self-Test Complete!");
}
