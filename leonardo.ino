/*
  =================================================
  Arduino Leonardo Full Self-Diagnostic Test
  =================================================

  Description:
  This program performs a **comprehensive self-diagnostic test** 
  on an **Arduino Leonardo** to verify the functionality of key components, including:
...
bool spiOK = false;
bool usbOK = false;
//bool i2cOK = false;  // Uncomment if using an I2C device
//bool voltageOK = false;   // Uncomment if measuring voltage

void setup() {
    Serial.begin(9600);
    while (!Serial);  // Wait for Serial Monitor to open (Leonardo specific)
    delay(2000);
    Serial.println("\n=== Arduino Leonardo Full Self-Diagnostic Test ===");
    ...
    Keyboard.end();
    usbOK = true;
    Serial.println("✔ USB HID Functionality: PASS");
    ...
    Serial.println(usbOK ? "✔ USB HID Functionality: OK" : "✖ USB HID Functionality: ERROR");
    //Serial.println(voltageOK ? "✔ 5V Power Supply: OK" : "✖ 5V Power Supply: ERROR");

    if (serialOK && ledOK && digitalIOOK && analogOK && pwmOK && eepromOK && spiOK && usbOK) {
        Serial.println("\n✅ Arduino Leonardo is fully functional!");
    } else {
